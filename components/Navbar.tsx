...
const Navbar = ({includeFilter=true, includePostBtn=true}: NavbarProps) => {
  ...
  useEffect(() => {
    if (pathname === "/" || startDate != null || endDate != null || distance != "Search Properties" || searchQuery) {
      handleFilterChange();
    }
  }, [startDate, endDate, distance, searchQuery]);
  ...
  return (
    <div className='items-center flex flex-row place-items-center justify-between w-full' style={{height: "15vh"}} >
      ...
      {includeFilter && (
        <div className="max-w-[780px] flex h-[78px] border-[2px] border-[#D9D9D9] rounded-[50px] shadow-lg flex flex-row place-items-center justify-between whitespace-nowrap mx-3 relative">
          <AnimatePresence>
            {isSearchOpen ? (
              <motion.form
              ...
              >
                <input
                  ...
                  className="flex-1 outline-none bg-white text-lg text-gray-700"
                  autoFocus
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                ...
                <button type="submit" className="ml-4">
                  <FaMagnifyingGlass className="h-5 w-5 text-[#FF7439]" />
                </button>
                <button 
                  type="button" 
          ...
          <div className={`w-full flex flex-row items-center justify-between ${isSearchOpen ? 'invisible' : ''}`}>
            {/* Distance from Rice */}
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button className="text-left self-start">
                      <p className={`text-base ${startDate && startDate.toDateString() !== "Select Start Date" ? "text-[#FF7439] font-semibold" : "text-[#777777] font-light"}`}>
            ...
            <div className="flex justify-center items-center flex-col w-[212px]">
              <div className="text-left">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button className='text-left'>
                      <p className={`text-[16px] ${endDate && endDate.toDateString() !== "Select date" ? "text-[#FF7439] font-semibold" : "text-[#777777] font-light"}`}>
            ...
            <button className='pr-8' onClick={() => setIsSearchOpen(true)}>
